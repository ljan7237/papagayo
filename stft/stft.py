...
#     # Save magnitude and phase as .npy files
#     magnitude_output_path = os.path.join(output_dir, filename.replace(".mp3", "_magnitude.npy").replace(".wav", "_magnitude.npy"))
#     np.save(magnitude_output_path, magnitude.astype(np.float32))

#     if save_phase:
#         phase_output_path = os.path.join(output_dir, filename.replace(".mp3", "_phase.npy").replace(".wav", "_phase.npy"))
#         np.save(phase_output_path, phase.astype(np.float32))
...
#     # Get all valid audio files
#     audio_files = [os.path.join(audio_dir, f) for f in os.listdir(audio_dir) if f.endswith((".mp3", ".wav", ".WAV", ".MP3"))]
    
#     # Parallel processing with tqdm progress bar
#     with Pool(num_workers) as pool, tqdm(total=len(audio_files), desc="Processing", unit="file") as pbar:
#         for _ in pool.imap_unordered(lambda file: process_audio(file, output_dir, n_fft, hop_length, save_phase), audio_files):
#             pbar.update()
